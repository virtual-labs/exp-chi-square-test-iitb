function activity2_p2() {
    ...
    btn && btn.remove();
    let btn_txt = get_collapse_btn_text('Calculate &chi;<sup>2</sup>', 'act2-p2-div');
    chi_sq_a2 = 0;
    chi_sq_a2 = parseFloat((((N - 1) * S_2_a2) / sigma_sq).toFixed(3));
    maindiv.innerHTML += `
      ${btn_txt}
      <div class='collpase divide center-text' id=act2-p2-div>
         ...
         <div class="fs-18px">
            ...
            <div id="act2-chi-2-div">
               <div class="row justify-content-center" style="align-items:center;">
                  <div class="col-md-4">
                     $$ \\chi^2_0 = \\frac{\\left(n-1 \\right)S^2}{\\sigma^2} =  $$
                  </div>
                  <div class="col-md-4">
                     <input type='number' id='act2-chi-2-inp' class='form-control fs-16px' />
                  </div>
               </div>
               <button class='btn btn-info std-btn' onclick='a2_verify_chi_2();' style='position: relative; left: 0w;' id='act2-vf-chi-2-btn'>Verify</button>
            </div>
         </div>
      </div>
   `;
...
function a2_verify_chi_2() {
    let chi_inp = (document.getElementById('act2-chi-2-inp'));
    console.log(chi_sq_a2);
    if (!verify_values(parseFloat(chi_inp.value), chi_sq_a2)) {
    ...
    let div = (document.getElementById('act2-chi-2-div'));
    div.innerHTML = '';
    div.innerHTML = `
      <div>
